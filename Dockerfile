...
RUN export DEBIAN_FRONTEND=noninteractive &&\
    ...
    sed -i "s#deb http://deb.debian.org/debian stable main#deb http://deb.debian.org/debian stable main non-free#g" /etc/apt/sources.list &&\
    apt-get -q update &&\
    apt-get install -qqy python3 python-cheetah python-sabyenc python-cryptography par2-tbb unrar p7zip-full unzip openssl python-openssl ca-certificates curl &&\
